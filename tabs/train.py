...
def _validate_filelist_dims(filelist_path: str):
    """
    Check that all feature files in the filelist have consistent dimensions.
    If mute features from a wrong embedder are present, auto-generate the right ones.
    """
    ...
    # sample up to 50 real features to determine the expected dim
    expected_dim = None
    for line in lines:
        parts = line.split("|")
        feat_path = parts[1]
        if "mute" not in feat_path and os.path.isfile(feat_path):
            expected_dim = np.load(feat_path).shape[-1]
            break

    if expected_dim is None:
        return  # no real features to compare against
    ...
    for line in lines:
        parts = line.split("|")
        feat_path = parts[1]
        if "mute" in feat_path and os.path.isfile(feat_path):
            feat_dim = np.load(feat_path).shape[-1]
            if feat_dim != expected_dim:
                # generate correct mute features
                mute_dir = os.path.dirname(os.path.dirname(feat_path))
                new_feat_path = os.path.join(
                    mute_dir, "extracted", f"mute_{expected_dim}.npy"
                )
                if not os.path.isfile(new_feat_path):
                    print(
                        f"  Regenerating mute features for dim={expected_dim}..."
                    )
                    _generate_mute_features_dim(
                        mute_dir, expected_dim, feat_path, new_feat_path
                    )
                parts[1] = new_feat_path
    ...
    if changed:
        print(f"  Updated {filelist_path} with corrected mute feature paths")
        with open(filelist_path, "w", encoding="utf-8") as f:
            f.write("\n".join(fixed_lines) + "\n")


def _generate_mute_features_dim(
    mute_dir: str, target_dim: int, old_feat_path: str, out_path: str
):
    """
    Generate mute features at target_dim by running the embedder on the mute audio.
    Uses the embedder from model_info.json in the experiment directory.
    """
    src_wav = os.path.join(
        mute_dir, "sliced_audios", os.path.basename(old_feat_path).replace(".npy", ".wav")
    )
    # fallback: look for any mute wav in the directory
    if not os.path.isfile(src_wav):
        wav_dir = os.path.join(mute_dir, "sliced_audios")
        candidates = [f for f in os.listdir(wav_dir) if f.endswith(".wav")] if os.path.isdir(wav_dir) else []
        if not candidates:
            # try the default mute
            default_wav = os.path.join(
                os.path.dirname(os.path.dirname(mute_dir)),
                "mute",
                "sliced_audios",
                "mute32000.wav",
            )
            if os.path.isfile(default_wav):
                src_wav = default_wav
            else:
                print(f"  Cannot find mute audio, skipping.")
                return
        else:
            src_wav = os.path.join(wav_dir, candidates[0])

    import torchaudio
    from rvc.lib.utils import load_embedding

    device = "cuda" if torch.cuda.is_available() else "cpu"
    # try to find embedder from model_info.json in parent experiment dir
    exp_dir = os.path.dirname(
        os.path.dirname(os.path.abspath(mute_dir))
    )
    if "logs" not in exp_dir:
        exp_dir = os.path.join(os.getcwd(), "logs")
    model_info = {}
    candidatess = sorted(glob.glob(os.path.join(exp_dir, "*", "model_info.json")))
    if not candidatess:
        # also try directly relative to mute_dir
        candidatess = [
            os.path.join(os.getcwd(), "logs", d, "model_info.json")
            for d in os.listdir(os.path.join(os.getcwd(), "logs"))
            if os.path.isdir(os.path.join(os.getcwd(), "logs", d))
        ]
        candidatess = [p for p in candidatess if os.path.isfile(p)]
    for c in candidatess:
        with open(c) as f:
            mi = json.load(f)
            if mi.get("embedder_dim") == target_dim:
                model_info = mi
                break
    embedder_name = model_info.get("embedder_model", "contentvec")
    try:
        model = load_embedding(embedder_name).to(device)
    except Exception as e:
        print(f"  Failed to load embedder '{embedder_name}' for mute generation: {e}, skipping")
        return

    model.eval()
    audio, sr = torchaudio.load(src_wav)
    if sr != 16000:
        resampler = torchaudio.transforms.Resample(sr, 16000)
        audio = resampler(audio)
    audio_16k = audio.mean(dim=0, keepdim=True).to(device)
    ...
    print(f"  Saved mute features: {feats.shape}")
...
# Validate and fix stale filelist entries (e.g. mute features from wrong embedder)
if os.path.isfile(config.data.training_files):
    _validate_filelist_dims(config.data.training_files)
...
def run(
    ...
    try:
        with open(model_info_path, "r", encoding="utf-8") as f:
            ...
            embedder_name = model_info.get("embedder_model", "contentvec")
            embedder_dim = model_info.get("embedder_dim", 768)
            spk_dim = model_info.get("speakers_id", config.model.spk_embed_dim)
    except Exception as e:
    ...
    config.model.text_enc_hidden_dim = embedder_dim
    ...
    ref_dir = embedder_name.replace("/", "_")
    ref_path = os.path.join("logs", "reference", ref_dir, "feats.npy")
    if os.path.isfile(ref_path):
        print("Using", ref_dir, "reference set for validation")
        phone = np.load(ref_path)
